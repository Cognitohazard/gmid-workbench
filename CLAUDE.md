...
- **Scope rule.** In scope: anything that is a pointwise or family-wise function of the device lookup tables plus a few PDK constants, and the algebraic consequences of declared connectivity — a shared node, Kirchhoff's current law at a node, Kirchhoff's voltage law around a loop. Stating those laws and propagating what they determine is physics, not solving. Out of scope: solving the node set simultaneously. The sizing engine **evaluates author-written expressions and checks author-written rules; it may propagate declared connectivity, but only in one bounded, non-iterative pass — it never iterates a coupled system to convergence.** A relation such a pass cannot settle without iterating is a fixed point: it must be reported, never silently closed, and the only fixed-point solving allowed is small, explicit, designer-named. The lookup-table substrate is the guardrail.
