...
export const CONTROL_HELP: Readonly<Record<string, string>> = {
  ...
  edges:
    'Two different questions get asked of a sheet, and this strip answers the second one. Does the design CLOSE — can this design-type meet the spec at all? The sweeps and the picker’s search work that way, free to re-size the devices while they look. Does it COVER — does the design you have right now still work at the ends of the range it claims (e.g. CM_dc = CM_lo, then CM_hi)? Every transistor is held exactly as this run sized it and only the bias re-settles, so a chip describes THIS instance and moves as you turn the knobs. Every hard rule must hold at each end. The badge folds both answers together, so a range end that does not cover turns it red with every rule still green — this strip is where you see which one failed. A green chip shows where the bias landed; a red one names what stopped it — and a coverage failure often yields to re-centering rather than to a bigger device, so read the sensitivity ranking before spending area. A dimmed chip means the question was not asked at all: the current settings produce no design to check it against. The sweep remains the authority on the full interior landscape.',
  paramRole:
